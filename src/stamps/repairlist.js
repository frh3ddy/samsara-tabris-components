...
const ContainerContentMethods = methods({
    addRepairName(text){
        this.repairName = this.createText(text, 'left')
    },
    addRepairPrice(text){
        this.repairPrice = this.createText(text, 'right')
    },
    updateRepairPrice(text) {
        this.repairPrice.set('text', text)
        this.updateSize()
    },
    ...
    animate() {
        ...
        const group = [ this.topBorder, this.bottomBorder, this.repairName ]
...
const Actions = init(function() {
    ...
    const editComposite = new Composite({
        ...
        const title = this.repairName.get('text')

        navigator.notification.prompt(
            `enter new price`, // message
            results => {
              if (results.buttonIndex === 1 || results.input1 === '') return
              this.repairPrice.set('text', results.input1)
            }, // callback to invoke
            title, // title
    ...
    const deleteComposite = new Composite({
        ...
        const repairName = this.repairName.get('text')
        var options = {
            addCancelButtonWithLabel: "Cancel",
            addDestructiveButtonWithLabel: `Remove ${repairName}`
        };
