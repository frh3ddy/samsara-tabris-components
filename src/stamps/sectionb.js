...
export default init(function({headerTitle, rows, parent}) {
    ...
    rows.forEach((data, index, array) => {
        ...
        if (index === 0 ) container = Container({border: '#dddfe6 1 0 0 0'}).appendTo(parent)
        if (index === array.length -1) container = Container({border: '#dddfe6 .5 0 1 0'}).appendTo(parent)

        if (index > 0 && index < array.length -1) {
            container = Container({border: '#dddfe6 .5 0 0 0'}).appendTo(parent)
        }
